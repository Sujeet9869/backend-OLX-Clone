...
app.post("/add-product", upload.fields([{name : 'pimage'},{name : 'pimage2'}]), (req, res) => {
    ...
    const newProduct = new Product({ pname, pdesc, price, category, pimage , pimage2, addedBy,
        pLoc : {
...
let longitude = req.query.loc.split(',')[1];   
    ...
    Product.find({
        $or: [
            { pname: { $regex: regexSearch } },
            { pdesc: { $regex: regexSearch } },
    ...
    .then(results => res.json({ message: 'Success', products: results }))
    .catch(err => res.status(500).json({ message: 'Server Error', error: err }));
});
...
app.get('/my-profile/:userId' , (req,res) => {
    ...
    .then((result) => res.json({ message: 'Success', user : {
        email : result.email ,
